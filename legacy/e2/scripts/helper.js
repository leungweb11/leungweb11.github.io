...
document.addEventListener("DOMContentLoaded", function () {
	...
	function conf_colour() {
		...
		if (last_theme) {
			document.querySelector(":root").style.setProperty("--primary-colour", "var(--" + last_theme + "-primary-colour)");
			document.querySelector(":root").style.setProperty("--secondary-colour", "var(--" + last_theme + "-secondary-colour)");
			document.querySelectorAll(`fieldset > input[data-radio="${last_theme}"]`).forEach((each) => {
				each.setAttribute("checked", "");
			});
		} else {
			document.querySelectorAll(`fieldset > input[data-radio="purple"]`).forEach((each) => {
				each.setAttribute("checked", "");
			});
		}
		/* For all radio input. */
		document.querySelectorAll("fieldset > input[data-radio]").forEach((each) => {
			each.addEventListener("change", function () {
				...
				document.querySelector(":root").style.setProperty("--primary-colour", "var(--" + theme + "-primary-colour)");
				document.querySelector(":root").style.setProperty("--secondary-colour", "var(--" + each.getAttribute("data-radio") + "-secondary-colour)");
		...
		(async () => {
			const { value: nav } = await Swal.fire({
				...
				didRender: function () {
					...
					document.querySelector(":root").style.setProperty("--swal2-confirm-button-color", "var(--primary-colour)");
				},
				didClose: function () {
