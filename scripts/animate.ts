import { animate, engine, svg, utils, JSAnimation } from "animejs";
const no_animation = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
...
let animation: JSAnimation[] = [
	animate(svg.createDrawable("path[stroke='#000'], rect[stroke='#000'], ellipse[stroke='#000'], polygon[stroke='#000'], circle[stroke='#000']"), {
		draw: ["0 0"], // Instantly hides the drawing outline; invoked only if animation is off.
		duration: 0,
	}),
];
